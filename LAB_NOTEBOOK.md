...
Before seeing their exam results, the next exam-transfer candidate will be selected by **lowest original-Wikipedia development loss on the million-token pool** among fresh raw-Wikipedia AK/AO/AW runs within the per-worker $10 bound. Mixture runs are a separate data comparison. Planned exam follow-up: ordinary SFT, conditional three-action SFT + KL0.1, and answer-only RLVR + KL0.1, seeds42/123/2026, the same 289 training questions, 80 exposure cap and six development option permutations. The repeatedly used test is exploratory; selection is not based on its score.

BD's matched 98M H100 AdamW LR0.0012 control reaches test loss1.5824, versus Muon LR0.0012's1.5773. Most of the earlier apparent optimizer advantage was also available by increasing AdamW's learning rate. Muon LR0.002 scores1.5906;291M Muon LR0.0012 scores1.8010. B20098M Muon LR0.0012 scores1.4747 versus AdamW LR0.0006's1.4920, but these learning rates differ. No strong enough advantage to change the workshop optimizer or launch a long Muon run.

### Short-answer SFT: varied wording and literary pretraining

BF/BG completed the eight-wording control. Exact answers on 100 frozen, reworded questions about **training facts**: 98M random 6, 98M after ten-minute Wikipedia pretraining 55, 291M after ten minutes 47, 291M after historical 8,000-second Wikipedia pretraining 93. Development scores are 8/48/62/90. All received 30 mean fact presentations. This measures adapting known definitions to question wording, not unseen knowledge or general instruction following.

The 291M historical model's earlier single-wording control reached only 19.6 presentations before its 600-second cap, versus 30 here; do not attribute its 69→93 change entirely to augmentation. BK repeats single-wording training with a 1,200-second cap and 30 presentations to remove this confound. The 98M ten-minute comparison already matches 30 presentations: 29→55.

Literal first examples for 291M Wikipedia→varied-definition SFT: “Co oznacza nazwa «Maserati»?” → “Włoski producent samochodów sportowych i wyścigowych.” Correct; but “Góry Dynarskie” → “Łańcuch górski na Półwyspie Sudetskim.” Wrong: reference says Bałkańskim. Broad questions still fail: “Co wiesz o Krakowie?” produces a university-unit definition. Preserve failures alongside successes.

BH/BJ compare matched ten-minute, 29.9M-parameter pretraining on Wolne Lektury and Wikipedia, then the same varied-definition SFT (30 presentations). Wolne Lektury scores 51/100 development and 53/100 test; Wikipedia 38/100 and 36/100. SFT worker estimates $0.419/$0.407, evaluations $0.022/$0.020. The QA facts are supplied during SFT; literary pretraining need not already contain them. Single seeds and narrow definition tasks do not establish a universal corpus ranking.

### Participant Wikipedia command and full-prose data

Added the optional `wiki-100m` recipe (98.3M actual parameters, H100, batch64, context512, peak LR0.0004). First complete participant checks, including diagnostics and reload verification: H100 676s/$0.784, 270.3M token presentations, test loss1.6484; B200 690s/$1.240, 392.8M presentations, test loss1.5735. Training budget600s, same fixed16k evaluation pool. These differ from research LR0.0006 measurements. Both live charts and final reports work; saved weights reproduce results after reload.

Removed the participant worker's intermediate first-$1 diagnostic (before/final/selected remain) and fixed periodic volume commits to the intended five-minute cadence. Research checkpoint diagnostics are unchanged. A second H100/B200 pair validates this exact participant path; allow for compilation and infrastructure variability when comparing timings.

Full-body prose preparation completed in2,002s using six parsing workers on eight CPU cores:1,081,446,123 training tokens in1,592,486 articles;688,969 source entries excluded by prose filters. Development/test:5,361,918/5,472,296 tokens. Same8k tokenizer and original article splits; exact cleaned-text deduplication prioritizes test, then development, then training. CPU estimate about$0.84 excluding startup; canceled serial work remains budgeted. BC training began only after the dataset manifest existed; BI will evaluate its final weights on the same fixed million-token corpus pools.

Participant recheck after the diagnostic/commit change: `scratch-wiki-100m-100m-1789693484571502092` (H100) 663.2s/$0.7685,302.1M token presentations,test1.6270; `scratch-wiki-100m-100m-1789693488143968251` (B200)659.9s/$1.1862,564.6M presentations,test1.5092. Both use LR0.0004,600s training and identical16k evaluation pools; both pass fresh-process sample reproduction and retain before/final/selected quality records plus browser reports. The live chart had already been verified for this command. These are single runs; do not attribute all throughput variation solely to the code change. B200's literal Warsaw continuation calls it an asteroid, while H100 calls it part of Warsaw: factual reliability remains poor. Main guide now gives this tested100M option and retains the cheap10M option.

BL's equal-exposure recall evaluation was submitted without its dependency and failed immediately because BK had not finished (worker estimate$0.00259). BM corrects this with `depends_on`. Added optional, validated `worker_timeout_seconds` for single-stage post-training/evaluation; the same value controls both Modal hard timeout and budget reservation. Training must retain at least300s for loading/evaluation; chained stages cannot use this override. Existing plans are unchanged. This avoids reserving a full GPU hour for bounded short follow-ups; it does not reduce reported actual costs. All historical plans and valid/invalid timeout-boundary checks pass.

### Fresh near-$10 runs, first common-pool results

AK's8,000-second H100 runs completed within their limits. 98M raw:4.463B presentations,$9.340,16k test1.2606;291M raw:$9.379,16k test1.2269. Both select the final checkpoint. On the fixed1,048,576-token pool, raw98M dev/test1.2218/1.2270 and291M1.1896/1.1975. The latter improves on the old291M133+50-minute continuation's1.2015 at lower total compute cost. These are different recipes, not a single uninterrupted learning curve.

98M with25% Wolne Lektury scores raw-Wikipedia test1.2755 and literature2.6316, versus raw-only1.2270/3.5089: a clear domain tradeoff. 12.5% popular-article oversampling scores raw1.2812 and clean-lead2.6396, worse than raw-only1.2270/2.4150. No benefit justifying either as the default Wikipedia recipe.

AW's B200 runs completed5,000s each:98M4.949B tokens/$9.088/16k test1.2511;wide100M5.742B/$9.074/1.2598;291M1.914B/$9.132/1.2143. All select final weights. Literal Warsaw continuations still describe a district, village or railway stop, respectively. Better loss is not evidence of reliable free-generation facts. Million-token evaluations are collected separately before candidate selection.

Prepared, not yet launched, BN: matched83-minute B200 runs with higher peak learning rates (98M0.0012 and291M0.0006). Short pilots suggested learning rate accounted for much of the apparent Muon advantage; this tests whether that survives a longer budget. No additional optimizer variant is needed for this hypothesis.

BK/BM resolves the augmentation confound:291M historical133-minute Wikipedia base, **30 presentations/fact in both conditions**, final weights. Single wording scores64/100 development and72/100 test; eight wordings90/100 and93/100. Single-wording SFT worker$1.0238; varied$1.0220. The earlier69/100 single-wording score used19.6 presentations; its matched30-presentation replacement remains substantially behind93. The recall chart now uses the matched control. Both refer to the same frozen known-fact probes and exact-match normalization; no unseen-knowledge claim.

Final AW million-token raw-Wikipedia dev/test:98M1.2149/1.2213;wide98M1.2218/1.2280;291M1.1754/1.1838. Standard291M/B200 currently leads these raw-corpus candidates. Shuffled H100 controls must finish before the predeclared exam-transfer selection. BN higher-LR runs and BO dependent evaluations are now detached remotely, with combined full-timeout reservations below the total$400 cap.

All seven pure raw-Wikipedia candidates finished their fixed million-token evaluations. Predeclared development-loss selection chooses `night-1789689694697165000-300m-B200-5000s` (dev 1.1753707090392709). BP/BQ/BR hold the fixed three-method, three-seed exam comparison; seed waves are submitted as completed jobs release budget. Hard timeout900s per worker,600s training cap; all62 earlier single-stage600s exam workers finished within654s. Test scores were not used to select the base.

### Long-run stopping trend and exam transfer

Within AW's291M/B200 run, the same16k development pool scores1.8313 at10min,1.5866 at30min,1.4186 at60min,1.3558 at80min and1.3538 at83.3min. These are checkpoints of one83-minute learning-rate schedule, not separately optimized shorter runs. Improvements diminish but have not stopped. H100291M's corresponding10/30/60/80/110/130/final133-minute scores are1.9733/1.6991/1.5425/1.4807/1.3992/1.3733/1.3693.

BP/BQ/BR completed the declared exam comparison on the development-selected291M/B200 base. Seeds42/123/2026, original/rotated test scores: ordinary SFT26/21,21/24,22/20; conditional3-letter SFT+KL25/22,25/21,26/20; RLVR21/21,21/19,23/23. Untuned base15/40 on original order. All nine metric/reload/development-selection audits pass. Better Wikipedia loss did not improve this small exam benchmark; retain that negative result. Each run completed80 mean presentations (1,445 updates), usually about2min and$0.14–0.21, rather than needing its full600s cap.

BS now compares the same varied-definition SFT on the best raw291M model and the two fresh full-prose models. It also trains general Polish OWCA instruction SFT on the291M prose model. BT waits for these checkpoints and evaluates known-fact recall or the fixed general-instruction probes. BU/BV/BW are the planned three-seed driving-exam comparison for291M prose, directly versus after instruction SFT; SFT and RLVR in each condition, identical earlier exam settings. Only BU is launched so far; further waves require budget release.

Budget accounting now releases completed siblings inside a batch with canceled calls, while keeping the **full per-worker timeout bound** for each unknown-cost call and the full controller allowance. Manifest order, run IDs and recomputed batch bounds are checked before doing so. Earlier S/Y calls remain fully reserved where billing is unknown; they are not treated as free. Completed CPU preparation replaces its maximum reserve with$4.50 including canceled work and margin. All active/queued jobs remain within$400.

### Fresh definition-recall audit, frozen before inference

`additional/scripts/prepare_qa_audit.py` selects200 other training facts, disjoint from the original200 probe facts, using stable SHA256 article-ID order. Ten newly written question/statement templates; no exact prompt appears in the eight-wording training set. Maximum question+answer length80 tokens, fitting all compared contexts. Audit SHA256 `35d3e0b901b3f212fb7cac8660c1ba542b8394f2ac2738b285801cad0031a3d6`. Both100-row halves retain legacy dev/test labels for the evaluator, but **neither half selects or tunes anything**. These are still known training facts, not unseen knowledge. BX will compare the historical291M varied-SFT checkpoint, the new raw291M checkpoint and new prose291M checkpoint, using predetermined final SFT weights. No training recipe changes will follow from this audit.

Interpretation: held-out-article definition loss and recall of known facts under new wording measure different objectives. Longer SFT can worsen the first while improving the second. The93/100 result uses final weights after a fixed30-presentations budget, not the checkpoint selected by held-out-article answer loss. For a closed catalogue of definitions, robustness to new question wording is the relevant additional check; this does not establish broad instruction following.

BC's fresh full-prose runs completed83.3min on B200:98M5.068B token presentations,$9.056,own-corpus16k test1.9776;291M1.985B,$9.086,test1.9115. Both select final weights and pass reload checks. These losses are not directly comparable with raw-markup loss. Fixed “Warszawa –” / “Polska –” continuations mostly produce route lists or administrative boilerplate, not useful encyclopedia definitions. Shared-corpus evaluation and SFT follow-ups are separate.

The new raw291M/B200 model followed by the same30-presentation varied-definition SFT scores81/100 development and86/100 test on the original frozen rewording probes, versus90/93 for the historical291M base. Same architecture and seed, but new pretraining processes1.914B tokens versus1.096B earlier and uses compilation/B200. Lower pretraining loss again does not automatically improve this fixed downstream recipe. Fresh-audit results remain pending. The fresh200-fact audit has198 distinct normalized answers; predicting the most frequent training answer scores2/200.

Fresh-audit after-SFT results: historical raw291M190/200, new raw291M190/200, new full-prose291M197/200. The earlier93-versus86 gap on the original100 test questions did not repeat; do not rank the two markup bases generally from that earlier gap. The prose result is promising but remains one SFT seed and known-fact recall. Original rewording probes for prose:98M88/100 development,91/100 test;291M90/89. Hardware checks: all four BS post-training workers actually used H100. Their training times differ (raw291M815s, prose291M630s, prose98M435s) despite equal30 fact presentations; do not interpret time differences as an isolated data effect.

General prose→OWCA instruction SFT took344.5s/$0.474 for three passes. It answers the capital question with “Stolicą Polski jest Warszawa.” but fails the one-word constraint; says2+3=12; repeats “Dzień dobry!” five times for exact-copy; swaps Gdańsk for Poznań in the reading task; describes Mickiewicz as Russian. More fluent response formatting is not dependable instruction following.

BN and AW share83-minute training budgets, architecture/batch/context and B200 pricing, but BN also records intermediate generation diagnostics and extra selected-checkpoint corpus diagnostics. Account for that overhead when comparing token counts and costs; do not attribute every difference solely to learning rate.

BY completed matched before-SFT inference on the exact fresh200-question audit: all three bases score0/200 under the same question-answer prefix and decoding. After definition SFT:190/200,190/200,197/200. This does not mean the bases contain no relevant knowledge: SFT changes both the prompt format they can use and their exposure to the supplied facts. The concise generated artifact is `results/wiki-qa-example-results.md`; source-linked Clock success and Cement failure are unchanged model outputs, with explicit truncation only for repetitive baselines.

Full-prose exam comparison, seeds42/123/2026, original/rotated scores: direct SFT28/20,24/18,24/20; direct RLVR24/16,24/21,24/13; instruction→SFT26/21,25/18,23/22; instruction→RLVR21/16,21/16,24/19. Baseline original-order scores:15/40 before direct exam tuning,12/40 after the instruction stage. All12 saved-metric audits pass. General instruction SFT did not provide a consistent exam advantage; option-order sensitivity remains large. Keep the stronger existing Qwen3.5-0.8B recipe as the practical driving-exam exercise.

### Final higher-learning-rate comparison

BN/BO completed both83-minute B200 runs and their fixed million-token evaluations. 98M at peakLR0.0012:4.882B token presentations,worker$9.1043,dev1.242210/test1.248737;291M at peakLR0.0006:1.854B,$9.1936,dev1.180243/test1.189625. Both select final weights and pass fresh-process reload checks. Separate evaluation workers cost$0.0584/$0.0984. Higher learning rates do not beat the original AW recipes (98M test1.221345;291M1.183783). AW291M remains the lowest-development-loss model among all nine fresh raw-Wikipedia candidates near$10. Extra BN diagnostics slightly reduce training-token throughput, so this remains an equal-time recipe comparison, not an isolated LR ablation.

Literal raw generations still fail: BN98M calls Warsaw a village in Gostynin municipality; BN291M calls it a railway stop and subsequently mixes bridge/tram descriptions. More training and lower next-token loss have not established reliable factual answering. Definition SFT provides the visible narrow success documented separately; general instruction SFT and driving-exam transfer remain weak.

All continuation batches now have complete manifests, including dependent evaluations. The report contains351 completed workers across the recorded sweeps (runs, not distinct architectures), totaling$273.1782 in recorded worker estimates. Conservative overnight ledger exposure is$381.89/$400, including prior-work allowance, CPU preparation, participant checks, controller bounds and full reservations for unknown canceled-call billing. This is a bound, not an invoice; completed-worker totals alone understate the total. No further paid jobs launched. The nine-candidate plot includes all final runs, and the selected recipe remains unchanged.

### Unified training visualization

`pnpm visualization` opens a local, read-only app with Tokenization, Pretraining, SFT and RLVR sections. The existing BPE explorer is embedded unchanged. One chart, checkpoint slider and fixed before/after prompts per training view; SFT exposes training inputs/targets and A/B/C probabilities, and RLVR shows recorded samples, verifier rewards and leave-one-out advantages. Saved examples are exported from the real participant runs into `visualization/examples.json`; large raw runs remain ignored. No model or Modal account is needed to view them.

Live metric messages now also carry small example snapshots. Scratch training records probabilities for the first two fixed prompts; RLVR records the first fixed development prompt, with other examples kept as text. These use the full-vocabulary next-token distribution. Scratch traces separately retain the temperature/top-k sampling probability. A/B/C bars remain explicitly conditional on those three letters, with their full-vocabulary mass reported separately. Byte-level records preserve split UTF-8 characters; coloring never inserts spaces or changes line layout. Missing historical probabilities are not invented.

Validation runs: `scratch-wolne-lektury-30m-1789724828611905521` (60s H100 training, worker$0.131894); `prawko-sft-1789724833387124975` (one epoch L4, $0.019886); `rlvr-train-six_words-1789724839751990099` (one RLVR update L4, $0.020743). All saved-model reload checks pass. These validate the interface/instrumentation, not improved model quality. New scratch traces reconstruct the exact saved text, and CPU checks establish that optional probability tracing preserves sampled token IDs.
