...
def evaluate_quality(model, tokenizer, device, outpath, *, max_new_tokens=128):
    ...
    try:
        ...
        with torch.no_grad(), torch.random.fork_rng(devices=cuda_devices):
            ...
            for index, prompt in enumerate(FREE_PROMPTS):
    ...
    result = dict(
        ...
        description='Ten fixed, curated factual preferences and eight free continuations; not a benchmark.',
        limitations=[
