...
def run(output, method='screen', model_key='qwen3.5-0.8b', max_seconds=180,
    ...
    if method not in ('screen', 'sft', 'rlvr') or not 60 <= max_seconds <= 600 or not 1 <= epochs <= 20:
        raise ValueError('Use screen/sft/rlvr, 60–600 seconds, 1–20 epochs')
    if not 1e-6 <= lr <= 5e-4:
        raise ValueError('Learning rate out of bounds')
    started = time.monotonic()
    out = Path(output)
    out.mkdir(parents=True, exist_ok=False)
    data_path = ROOT / 'datasets/prawko-v1/data.json'
    ...
    result = dict(method=method, model_spec=spec, seed=seed, data_sha256=hashlib.sha256(data_path.read_bytes()).hexdigest(),
        ...
        training_data='101 official category-B text-only three-choice questions; separate 24 dev and 40 test', before=before)
    if method != 'screen':
        ...
        result.update(after=after, training_seconds=train_seconds, steps=len(history),
