...
def generate(model, tokenizer, stop, prompts, device, system=None, sample=False, max_tokens=192):
    ...
    if sample:
        config.temperature, config.top_p, config.top_k = .8, .95, 40
    model.eval()
...
def train(data_dir, output, model_key='gemma4-e2b', epochs=6, max_seconds=300, device='cuda', lr=1e-4):
    import torch
...
if __name__ == '__main__':
    ...
    p.add_argument('stage', choices=['data', 'train', 'chat'])
    p.add_argument('--style', choices=['poetry','wit'], default='poetry')
    p.add_argument('--model', choices=MODELS, default='gemma4-e2b')
    ...
    if a.stage=='data':
        ...
        if not a.adapter_dir:
            p.error('--adapter-dir is required for chat')
