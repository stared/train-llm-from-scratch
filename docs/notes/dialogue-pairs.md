...
`scripts/dialogue_pairs.py` converts a supplied local text file into ordinary `prompt`/`answer` JSONL and equivalent chat `messages`. For adjacent lines A then B, it creates **A → B** and **B → A**, always using the standard `user` and `assistant` roles. Speaker names are metadata rather than part of the utterance.
...
```bash
uv run scripts/dialogue_pairs.py my-dialogues.txt --output datasets/my-dialogues --source 'Description of the supplied source'
```
...
The training JSONL is compatible with `scripts/style_workshop.py`. Once there are at least 24 training examples:

```bash
uvx --from modal==1.5.0 modal run scripts/style_modal.py --stage train --data-run my-dialogues --model qwen3.5-4b --epochs 4 --max-seconds 300
```
...
**Later supplied-file experiment:** `scripts/prepare_chlopaki.py` handles the headings, abbreviated speakers and unlabelled continuation lines in the user's `datasets/chlopaki.md`. See [docs/notes/chlopaki.md](chlopaki.md) for the actual 1,270-example dataset and training recipe.
