...
For the newer minute-scale task comparison, see [RLVR workshop examples](../rlvr.md), [all before/after outputs](../../results/rlvr-results.html), and [what the failed runs teach about rewards](../../results/rlvr-reward-lesson.md). The old pipeline below changed adapter weights but remained at 4/8 accuracy.
...
uvx --from modal==1.5.0 modal run scripts/rlvr_modal.py --sft-run YOUR_SFT_RUN --groups 12 --rollouts 4
```
