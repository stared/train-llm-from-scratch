...
**Longer follow-up:** [interactive learning curves and actual choices](../results/prawko-training.html), [all selected/final before–after results](../results/prawko-long-results.md). After12 minutes each, final SFT scored28/40 (reordered30/40), final RLVR25/40 (reordered24/40). Development-selected checkpoints reversed the small original-order lead: SFT28/40, RLVR29/40. See the full comparison below.
...
[Every before/after choice, including regressions](../results/prawko-example-results.md). This is a modest pilot improvement, not a dramatic exam-passing claim. For a workshop, use it to compare SFT and RLVR; the six-word task remains the stronger visible RLVR success.
...
```bash
uv run scripts/prawko.py --method sft --epochs 10 --max-seconds 180 --device cuda --output runs/my-prawko-sft
uv run scripts/prawko.py --method rlvr --epochs 10 --max-seconds 180 --device cuda --output runs/my-prawko-rlvr
```

CPU is the local default; CPU/MPS training has not been benchmarked. No notebook or coding assistant is required. `scripts/prawko.py.lock` pins dependencies. The Modal wrapper uses the same trainer. Small run records are copied back to `runs/`; weights stay in the `model-training-workshop` Modal volume under `/runs/RUN_NAME/adapter`.
...
uv run scripts/prepare_prawko.py data/prawko/source.xlsx --output data/prawko-rebuilt
...
```bash
uvx --from modal==1.5.0 modal run scripts/prawko_modal.py --method compare --max-seconds 720 --epochs 40
```
...
```bash
uv run scripts/verify_prawko.py runs/RUN_NAME
```
...
```bash
uv run scripts/prawko_learning_report.py runs/prawko-sft-1788861428320342529 runs/prawko-rlvr-1788862202341560425
uv run scripts/prawko_report.py runs/prawko-sft-1788861428320342529 runs/prawko-rlvr-1788862202341560425 --output results/prawko-long-results.md
uv run scripts/verify_prawko.py runs/prawko-sft-1788861428320342529 runs/prawko-rlvr-1788862202341560425
```
