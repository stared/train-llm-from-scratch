...
Implementation of the v2 scratch experiment. The user explicitly requested original Wikipedia markup. The expanded comparison includes ten-minute training on full Wikipedia, filtered article openings, popular article openings, historical Wolne Lektury, and TinyStories. See [the expanded curves and samples](../results/pretraining-results.html), [generated examples](../results/pretraining-example-results.md), and [experiment conclusions](../results/pretraining-findings.md). The [original five-minute comparison](../results/wiki-scratch-results.html) remains available.
...
```bash
uv run scripts/train_scratch.py --size 10m --max-seconds 300 --device cuda --output runs/my-scratch-10m
uv run scripts/train_scratch.py --size 30m --max-seconds 300 --device cuda --output runs/my-scratch-30m
uv run scripts/sample_scratch.py runs/my-scratch-10m --device cuda --output runs/my-scratch-10m/my-scratch-samples.json
```
...
```bash
uvx --from modal==1.5.0 modal run scripts/scratch_modal.py --size compare --max-seconds 300
```
...
Open [visualizations/tokenizer.html](../visualizations/tokenizer.html) directly in a browser. It shows the same monospaced text with changing token boundaries and one merge at a time; links to Cornell for a full algorithm lesson. Rebuild with `uv run scripts/tokenizer_visualization.py`; add `--text "Twój tekst"` for another real-tokenizer example.
