...
**My selection advice:** offer a cheap tested model for the first experiment, Qwen3.5 for a modern multilingual alternative, and LFM2.5-2.6B for participants who specifically want a very recent release. Gemma 4 is worth offering as an advanced alternative rather than pretending E2B is the same memory class as Qwen 2B. The actual tested combinations and failures belong in `results/test-results.md`; presence in `config/models.json` only means checkpoint metadata is pinned, not that the recipe is verified.
