...
- [Editable dataset source](../scripts/curated_data.py): `uv run scripts/curated_data.py` rebuilds the JSONL and hash manifests.
...
[Full untuned / 256-target / 2,048-target comparisons, longer-output probes and costs](pan-tadeusz-2048-results.md).

## Actual Pan Tadeusz Q&A fine-tune — 450 pairs, two passes

**Model: Qwen/Qwen3.5-4B**, revision `851bf6e806efd8d0a36b00ddf55e13ccb7b8cd0a`, fine-tuned with a fresh rank-16 LoRA on **450 ordinary Polish prompts → 4–12 original Pan Tadeusz verse lines**. These are actual model outputs, not the dataset targets. Training: 370.22 seconds, 225 updates, two full passes. Held-out prompts, no poetry instruction for base or adapter. Sampling: seed 42, temperature .7, top-p .8, top-k 20, repetition penalty 1.1, maximum 384 tokens. [Every output, including failures and prompt-only comparison](pan-tadeusz-qa-results.md).
