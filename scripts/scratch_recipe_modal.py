...
def execute_recipe(recipe, seconds, progress_queue=None):
    import sys
    ...
    return execute(size, seconds, .000222 if gpu == 'L4' else .001097,
                   ...
                   eval_interval=60, progress=reporter(progress_queue))

@app.function(image=image, gpu='L4', cpu=2, memory=16384, timeout=1000,
              retries=0, max_containers=1, volumes={'/persist': volume})
def cheap(recipe, seconds, progress_queue=None):
    return execute_recipe(recipe, seconds, progress_queue)

@app.function(image=image, gpu='H100', cpu=2, memory=16384, timeout=1000,
              retries=0, max_containers=1, volumes={'/persist': volume})
def fast(recipe, seconds, progress_queue=None):
    return execute_recipe(recipe, seconds, progress_queue)

@app.local_entrypoint()
def main(recipe: str = 'wolne-lektury', max_seconds: int = 600):
    ...
    name, files = run_live(worker, 'pretrain', recipe, max_seconds)
