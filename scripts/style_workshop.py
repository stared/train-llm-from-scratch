...
def dataset_metrics(text, manifest):
    metrics = surface_metrics(text, manifest['style'])
    if 'expected_line_range' in manifest:
        low, high = manifest['expected_line_range']
        metrics['style_shape'] = low <= metrics['nonempty_lines'] <= high
    return metrics


def dataset_instruction(manifest):
    if 'expected_line_range' in manifest:
        low, high = manifest['expected_line_range']
        return (f'Odpowiadaj wyłącznie po polsku, w {low}–{high} wersach, bez wstępu ani tytułu. '
                'Używaj stylu Pana Tadeusza Adama Mickiewicza. Na zwyczajne współczesne '
                'sytuacje odpowiadaj literacką sceną lub porównaniem; humor, anachronizm '
                'i przesadnie dramatyczna reakcja są mile widziane. Celuj w trzynastozgłoskowiec.')
    return instruction(manifest['style'])
...
def generate(model, tokenizer, stop, prompts, device, system=None, sample=False, max_tokens=192,
             temperature=.8, top_p=.95, top_k=40, repetition_penalty=1.0):
    import torch
...
def train(data_dir, output, model_key='qwen3.5-4b', epochs=4, max_seconds=300, device='cuda', lr=1e-4, batch_tokens=0, resume_dir=None):
    import torch
    from peft import LoraConfig, get_peft_model, PeftModel
    if batch_tokens not in (0, 1024, 2048, 4096):
        raise ValueError('batch_tokens must be 0, 1024, 2048 or 4096')
    ...
    started = time.monotonic()
    tokenizer, stop, load, spec = setup(model_key, device)
    if previous_result and previous_result['model_spec'] != spec:
        raise ValueError('Saved adapter model revision differs from current model specification')
    encoded = []
    for row in rows:
        ...
        limit = batch_tokens or 512
        if len(prefix)+len(response) > limit:
    ...
    def evaluate(model, label, system=None):
        ...
        for start in range(0, len(eval_rows), 4):
            ...
            answers = generate(model, tokenizer, stop, [r['prompt'] for r in batch], device, system,
                               max_tokens=384 if 'expected_line_range' in manifest else 192)
            results.extend({**r, 'answer': a, **dataset_metrics(a, manifest)} for r, a in zip(batch, answers))
    ...
    if resume_dir:
        ...
        write_json(out / 'base.json', before)
        write_json(out / 'prompted.json', prompted)
    else:
        before = evaluate(model, 'base')
        prompted = evaluate(model, 'prompted', dataset_instruction(manifest))
    ...
    if resume_dir:
        model = PeftModel.from_pretrained(model, resume_dir / 'adapter', is_trainable=True)
    else:
    ...
    for _ in range(epochs):
        epoch = [i for i in range(len(encoded)) if i not in previous_seen]
        rng.shuffle(epoch)
        schedule.extend(epoch)
    batches = []
    if batch_tokens:
        # Bucket within small shuffled pools, then shuffle batches: preserve all
        # examples while reducing padding and avoiding a short-to-long curriculum.
        for pool_start in range(0, len(schedule), 64):
            pool = sorted(schedule[pool_start:pool_start+64], key=lambda i: len(encoded[i][0]))
            batch = []
            for index in pool:
                if batch and (len(batch)==4 or (len(batch)+1)*len(encoded[index][0])>batch_tokens):
                    batches.append(batch); batch=[]
                batch.append(index)
            if batch: batches.append(batch)
        rng.shuffle(batches)
    else:
        batches = [schedule[i:i+4] for i in range(0,len(schedule),4)]
    history = []; seen = set(previous_seen); examples_seen = 0
    training_start = time.monotonic()
    for indices in batches:
        optimizer.zero_grad(set_to_none=True)
        avg = 0.
        if batch_tokens:
            length = max(len(encoded[i][0]) for i in indices)
            input_ids = torch.tensor([encoded[i][0]+[tokenizer.pad_token_id]*(length-len(encoded[i][0])) for i in indices], device=device)
            labels = torch.tensor([encoded[i][1]+[-100]*(length-len(encoded[i][1])) for i in indices], device=device)
            attention = torch.tensor([[1]*len(encoded[i][0])+[0]*(length-len(encoded[i][0])) for i in indices], device=device)
            loss = model(input_ids=input_ids, attention_mask=attention, labels=labels, use_cache=False).loss
            if not torch.isfinite(loss): raise RuntimeError('Non-finite training loss')
            loss.backward(); avg=loss.item()
        else:
        ...
        seen.update(indices); examples_seen += len(indices)
        torch.nn.utils.clip_grad_norm_(params, 1.)
        optimizer.step()
        history.append(dict(step=len(history)+1, loss=avg, examples_seen=examples_seen,
    ...
    write_json(out / 'seen_example_indices.json', sorted(seen))
    ...
    result = dict(model=model_key, model_spec=spec, style=style, examples=len(rows), epochs_requested=epochs,
                  ...
                  batch_tokens=batch_tokens, unique_examples_seen=len(seen), full_dataset_seen=len(seen)==len(rows),
                  gradient_checkpointing=bool(batch_tokens),
                  continued_from=str(resume_dir) if resume_dir else None,
                  baseline_reused_from=str(resume_dir) if resume_dir else None,
                  continuation_note='Adapter weights continued on previously unseen rows; AdamW restarted' if resume_dir else None,
                  cumulative_training_seconds=training_seconds+(previous_result.get('cumulative_training_seconds',previous_result['training_seconds']) if previous_result else 0),
                  loss_weighting="answer-token mean within batch" if batch_tokens else "example mean",
...
def chat(adapter_dir, output, prompt, device='cuda'):
    ...
    result = {'prompt': prompt, 'base': before, 'finetuned': after,
              'adapter_run': run.name, 'style': spec['style'], 'model': spec['model'],
              'model_spec': spec['model_spec'], 'training_data': spec['teacher'],
...
def sample_saved(adapter_dir, output, device='cuda', adapter_scale=1.0):
    ...
    def evaluate(label, system=None):
        ...
        for start in range(0, len(eval_rows), 4):
            ...
            texts = generate(model, tokenizer, stop, [r['prompt'] for r in batch], device,
                             ...
                             repetition_penalty=1.1, max_tokens=384 if 'expected_line_range' in manifest else 192)
            answers.extend({**r, 'answer': text, **dataset_metrics(text, manifest)}
                           for r, text in zip(batch, texts))
        write_json(out / f'{label}.json', answers)
        return sum(r['style_shape'] for r in answers)/len(answers)

    rates = {'base': evaluate('base'), 'prompted': evaluate('prompted', dataset_instruction(manifest))}
    model = PeftModel.from_pretrained(model, run / 'adapter')
    scaled_layers = 0
    for module in model.modules():
        if hasattr(module, 'lora_A') and hasattr(module, 'scaling'):
            for name in module.scaling:
                module.scaling[name] *= adapter_scale
            scaled_layers += 1
    ...
    result.update(source_run=run.name, evaluation_only=True, surface_shape_rates=rates,
                  adapter_scale=adapter_scale, scaled_layers=scaled_layers,
                  decoding={'sample': True, 'temperature': .7, 'top_p': .8, 'top_k': 20,
                            'presence_penalty': 0, 'repetition_penalty': 1.1, 'seed': 42, 'max_new_tokens': 384 if 'expected_line_range' in manifest else 192})
    write_json(out / 'evaluation.json', eval_rows)
    write_json(out / 'style_result.json', result)
    write_json(out / 'dataset.json', manifest)
    return result
...
if __name__ == '__main__':
    ...
    p.add_argument('--batch-tokens', type=int, default=0)
    p.add_argument('--resume-dir', help='Continue saved adapter on rows not yet seen; same dataset/model, one epoch')
    p.add_argument('--adapter-scale',type=float,default=1.0,help='LoRA strength for sample stage')
    ...
    if a.stage=='data':
        ...
        train(a.data_dir,a.output,a.model,a.epochs,a.max_seconds,a.device,batch_tokens=a.batch_tokens,resume_dir=a.resume_dir)
    else:
        ...
        if a.stage=='chat':
            ...
            sample_saved(a.adapter_dir,a.output,a.device,a.adapter_scale)
