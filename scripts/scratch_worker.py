...
def execute(size,seconds,gpu_rate,tag='workshop',data_name='wiki-scratch-v1',volume=None,batch_size=64,context_length=512,warmup_steps=100,eval_interval=300,progress=None,compile_training=False):
    import sys
    import subprocess
    import traceback
    ...
    source_info=json.loads((Path('/persist/datasets')/data_name/'tokens.json').read_text(encoding='utf-8'))
    last_commit=0.
    print('RUN',name,flush=True)
    def hook(model,tokenizer,stage,step,elapsed,path):
        nonlocal last_commit
        label=stage if stage in ('before','final','selected') else None
        if label:
            if label=='selected' and data_name in ('wiki-leads-v1','wiki-popular-v1'):
                ...
                (path/'common_wiki_eval.json').write_text(json.dumps(common,indent=2), encoding='utf-8', newline='\n')
            ...
            (path/f'quality_{label}.json').write_text(json.dumps(result,ensure_ascii=False,indent=2), encoding='utf-8', newline='\n')
        # Flush artifacts every five minutes; unique per-run paths allow parallel writers.
        if stage!='checkpoint' or elapsed-last_commit>=300:
            volume.commit();last_commit=elapsed
    try:
        result=run('/persist/datasets/'+data_name,out,size,seconds,42,'cuda',batch_size,
                   ...
                   warmup_steps=warmup_steps,checkpoint_hook=hook,progress=progress,compile_training=compile_training)
        ...
        result['fresh_process_reload_matches']=json.loads((out/'reload_samples.json').read_text(encoding='utf-8'))==json.loads((out/'samples_selected.json').read_text(encoding='utf-8'))
        ...
        result.update(remote_seconds=time.monotonic()-start,estimated_compute_usd=(time.monotonic()-start)*rate,
                      configured_hourly_usd=rate*3600,quality_checkpoint_policy='before, final, development-selected; periodic artifact commits every 300s')
        ...
        (out/'failure.json').write_text(json.dumps(dict(traceback=traceback.format_exc(),seconds=time.monotonic()-start,
                                                       estimated_compute_usd=(time.monotonic()-start)*rate)), encoding='utf-8', newline='\n')
        raise
    finally:
        if out.exists():
            for filename in ('scratch_model.py','train_scratch.py','sample_scratch.py','scratch_quality.py','scratch_worker.py'):
                (out/('executed_'+filename)).write_text((Path('/work/scripts')/filename).read_text(encoding='utf-8'), encoding='utf-8', newline='\n')
        volume.commit()
