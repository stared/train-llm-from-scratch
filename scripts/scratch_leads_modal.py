"""Ten-minute filtered Wikipedia opening comparisons; full-Wiki evaluation also saved."""
from pathlib import Path
import modal
volume=modal.Volume.from_name('model-training-workshop')
image=(modal.Image.debian_slim(python_version='3.12').pip_install_from_requirements('scripts/requirements.txt')
...
app=modal.App('workshop-scratch-leads')
...
def small():
    ...
    return execute('10m',600,.000222,tag='cheap',data_name='wiki-scratch-v1',volume=volume,batch_size=32,context_length=256,warmup_steps=20)
@app.function(image=image,gpu='H100',cpu=2,memory=16384,timeout=1000,retries=0,max_containers=2,volumes={'/persist':volume})
def larger(size):
    import sys
    sys.path.insert(0,'/work/scripts')
    from scratch_worker import execute
    return execute(size,600,.001097,tag='leads',data_name='wiki-leads-v1',volume=volume)
