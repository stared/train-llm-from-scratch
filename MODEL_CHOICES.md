...
**Verified recipes:** LFM2.5-350M (all three exercises), LFM2.5-2.6B (JSON extraction), Qwen3.5-0.8B (routing), and Gemma 4 E2B **instruction-tuned** (two-update compatibility check). Gemma must use `google/gemma-4-E2B-it`; the checkpoint without `-it` is a base checkpoint without a chat template. See [test results](TEST_RESULTS.md) for limitations and exact measurements.
...
**My selection advice:** offer a cheap tested model for the first experiment, Qwen3.5 for a modern multilingual alternative, and LFM2.5-2.6B for participants who specifically want a very recent release. Gemma 4 is worth offering as an advanced alternative rather than pretending E2B is the same memory class as Qwen 2B. The actual tested combinations and failures belong in `TEST_RESULTS.md`; presence in `models.json` only means checkpoint metadata is pinned, not that the recipe is verified.
