...
class PrawkoTests(unittest.TestCase):
    ...
    def test_split_and_similarity_isolation(self):
        data = json.loads(Path('datasets/prawko-v2/data.json').read_text(encoding='utf-8'))
    ...
    def test_expanded_data_preserves_holdouts(self):
        original=json.loads(Path('datasets/prawko-v2/data.json').read_text(encoding='utf-8'))
        extended=json.loads(Path('datasets/prawko-v2/extended.json').read_text(encoding='utf-8'))
