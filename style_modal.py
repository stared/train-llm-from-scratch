...
def experiment(stage,style,model,languages,data_run,epochs,max_seconds,prompt,batch_tokens,resume_run,adapter_scale):
    ...
    try:
        ...
        (path/'executed_style_workshop.py').write_text(Path('/work/style_workshop.py').read_text())
        (path/'executed_style_data.py').write_text(Path('/work/style_data.py').read_text())
