...
image = (modal.Image.debian_slim(python_version='3.12')
         ...
         .add_local_file('rlvr_showcase.py', '/work/rlvr_showcase.py')
         .add_local_file('rlvr_tasks.py', '/work/rlvr_tasks.py')
...
def experiment(task, stage, model, max_seconds, steps, lr, seed, beta, dev_interval):
    ...
    try:
        ...
        for filename in ['rlvr_showcase.py', 'rlvr_tasks.py']:
            (path / ('executed_' + filename)).write_text((Path('/work') / filename).read_text())
