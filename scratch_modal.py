...
image=(modal.Image.debian_slim(python_version='3.12')
    ...
    .add_local_file('scratch_model.py','/work/scratch_model.py')
    .add_local_file('train_scratch.py','/work/train_scratch.py')
    .add_local_file('sample_scratch.py','/work/sample_scratch.py'))
...
def experiment(size,max_seconds,batch_size):
    ...
    try:
        ...
        subprocess.run([sys.executable,'/work/sample_scratch.py',str(out),
        ...
        for file in ('scratch_model.py','train_scratch.py','sample_scratch.py'):
            (out/('executed_'+file)).write_text((Path('/work')/file).read_text())
