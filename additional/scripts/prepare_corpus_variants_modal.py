...
def prepare(variant='plain-leads'):
    ...
    if variant!='plain-leads':raise ValueError('Unknown corpus variant')
    start=time.monotonic()
    source=Path('/persist/datasets/wiki-leads-v1');out=Path('/persist/datasets/wiki-plain-leads-v2')
...
def main(variant: str = 'plain-leads'):
    ...
    name='wl-wiki-bpe-v1' if variant=='wolne-lektury-wiki-bpe' else 'wiki-plain-leads-v2'
