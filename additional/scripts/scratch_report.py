...
def render(paths):
    ...
    Path('results/wiki-scratch-results.html').write_text(HTML.replace('const runs=DATA;','const runs='+json.dumps(runs,ensure_ascii=False).replace('<','\\u003c')+';'))
    Path('results/wiki-scratch-example-results.md').write_text('\n'.join(md))
